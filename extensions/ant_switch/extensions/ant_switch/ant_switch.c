...
char * ant_switch_queryantennas() {
	...
	non_blocking_cmd("/usr/local/bin/ant-switch-frontend s", line, sizeof(line), NULL);
...
int ant_switch_setantenna(char* antenna) {
	...
	int n;
	char *antennastr;
        asprintf(&antennastr, "/usr/local/bin/ant-switch-frontend %s", antenna);
	line[0] = '\0';
...
int ant_switch_toggleantenna(char* antenna) {
	...
	int n;
	char *antennastr;
        asprintf(&antennastr, "/usr/local/bin/ant-switch-frontend t%s", antenna);
	line[0] = '\0';
	n = non_blocking_cmd(antennastr, line, sizeof(line), &status);
	free(antennastr);
	return(status);
